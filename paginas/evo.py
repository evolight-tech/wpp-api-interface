...
def extrair_data(arquivo):
    try:
        ...
        data_hora_str = (
            partes[-2] + " " + partes[-1].replace(".txt", "")
        )  # Junta data e hora e remove a extensão
        return datetime.strptime(
            data_hora_str, "%Y-%m-%d %H-%M-%S"
        )  # Novo formato com hífens e traços
...
def render():
    ...
    option = st.selectbox(
        "Escolha uma ação:", ["Ver Chats", "Listar Conversas", "Remover Chat"]
    )
    ...
    if option == "Ver Chats":
            ...
            if len(response) > 0:
                ...
                for index, arquivo in enumerate(response):
                    ...
                    if col2.download_button(
                        label="Baixar",
                        data=call_api(
                            f"/history/{arquivo}", method="GET"
                        ),  # Obtém o conteúdo diretamente
            ...
            if st.button("Baixar Todos os Arquivos", type="secondary"):
                ...
                with zipfile.ZipFile(zip_buffer, "w") as zip_file:
                    for arquivo in response:
                        file_data = call_api(
                            f"/history/{arquivo}", method="GET"
                        )  # Obtém o conteúdo de cada arquivo
        ...
        with col3:
            ...
            if st.session_state.get("confirmar_apagar_tudo", False):
                st.warning(
                    "Tem certeza de que deseja apagar TODOS os arquivos? Esta ação não pode ser desfeita."
                )
