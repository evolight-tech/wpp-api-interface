...
def render():
    ...
    option = st.selectbox(
        "Escolha uma ação:", ["Ver Itens", "Adicionar Item", "Remover Item"]
    )
    ...
    if option == "Ver Itens":
        if st.button("Listar FAQ"):
            ...
            if response and isinstance(
                ...
                if len(response) > 0:
                    ...
                    # Reordenar colunas
                    colunas_ordenadas = ["id", "pergunta", "resposta", "categoria"]
        ...
        categoria = st.selectbox(
            "Categoria", ["Mercado Livre de Energia", "Geração de Energia"]
        )
